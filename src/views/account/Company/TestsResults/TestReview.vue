...
const loadTestData = async () => {
  try {
    const attemptId = route.params.testId as string;
    if (!attemptId) {
...
const navigateToResults = () => {
  router.push(`/company/jobs?companyId=${userId}`);
};
  ...
  <div class="max-w-4xl mx-auto">
    ...
    <template v-else-if="testAttempt">
      ...
      <div class="mt-8 flex justify-center gap-4">
        <router-link
            :to="`/company/tests/${testAttempt.testId}/results?jobId=${testAttempt.jobId}`"
