...
<template>
  <div class="max-w-4xl">
    <div class="bg-gray-800 rounded-lg p-6">
      <h2 class="text-2xl font-bold mb-6">{{ $t("editTest.title") }}</h2>

      <form @submit.prevent="handleSubmit" class="space-y-6">
        <!-- Basic Information -->
        <div class="space-y-4">
          <div>
            <label class="block text-sm font-medium mb-2">{{ $t("editTest.testTitle") }}</label>
            <input v-model="formData.title" type="text" required class="w-full px-3 py-2 bg-gray-700 rounded-md" placeholder="Test Title" />
          </div>

          <!-- Job Selection -->
          <div>
            <label class="block text-sm font-medium mb-2">{{ $t("editTest.linkToJob") }}</label>
            <div class="space-y-2">
              <select
                  v-model="formData.jobId"
                  class="w-full px-3 py-2 bg-gray-700 rounded-md focus:ring-primary-500 focus:border-primary-500"
              >
                <option value="">{{ $t("editTest.selectJob") }}</option>
                <option v-for="job in jobs" :key="job.id" :value="job.id">{{ job.title }}</option>
              </select>

              <div v-if="formData.jobId" class="flex items-center gap-2 mb-2">
                <input
                    type="checkbox"
                ...
                />
                <label for="required-test" class="text-sm">{{ $t("editTest.requiredForJob") }}</label>
              </div>
            </div>
          </div>

          <div>
            <label class="block text-sm font-medium mb-2">{{ $t("editTest.description") }}</label>
            <textarea
                v-model="formData.description"
                required
                rows="3"
                class="w-full px-3 py-2 bg-gray-700 rounded-md focus:ring-primary-500 focus:border-primary-500"
                :placeholder="$t('editTest.descriptionPlaceholder')"
            ></textarea>
          </div>

          <div style="display: none" class="flex flex-col gap-2">
            <label for="video-record" class="text-sm font-semibold">{{ $t("editTest.videoRecordingRequirement") }}</label>
            <div class="flex items-center gap-2">
              <input
                  ...
                  class="rounded border-gray-600 text-primary-600 focus:ring-primary-500"
              />
              <span class="text-sm">{{ $t("editTest.enableVideoRecording") }}</span>
            </div>
            <p class="text-xs text-gray-400 mt-1">{{ $t("editTest.videoRecordingInfo") }}</p>
          </div>

          <div class="grid md:grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium mb-2">{{ $t("editTest.category") }}</label>
              <select
                  v-model="formData.category"
                  required
                  class="w-full px-3 py-2 bg-gray-700 rounded-md focus:ring-primary-500 focus:border-primary-500"
              >
                <option value="">{{ $t("editTest.selectCategory") }}</option>
                <option
            ...
            <div>
              <label class="block text-sm font-medium mb-2">{{ $t("editTest.difficulty") }}</label>
              <select
          ...
          <div>
            <label class="block text-sm font-medium mb-2">{{ $t("editTest.duration") }}</label>
            <input
          ...
          <div v-if="formData.questions.length > 0" class="space-y-4">
            <h3 class="text-lg font-semibold">{{ $t("editTest.questions") }} ({{ formData.questions.length }})</h3>

            <div v-for="(question, index) in formData.questions" :key="index" class="bg-gray-700 p-4 rounded-lg">
              <div class="flex justify-between items-start mb-2">
                <h4 class="font-medium">{{ $t("editTest.question") }} {{ index + 1 }}</h4>

                <div>
                  <button v-if="editableQuestionIndex !== index" @click="editQuestion(index)" class="text-blue-400 hover:text-blue-300">{{ $t("editTest.edit") }}</button>
                  <button v-else @click="saveQuestionEdit" class="text-green-400 hover:text-green-300">{{ $t("editTest.save") }}</button>
                  <button v-if="editableQuestionIndex === index" @click="cancelQuestionEdit" class="ml-2 text-red-400 hover:text-red-300">{{ $t("editTest.cancel") }}</button>
                  <button @click="deleteQuestion(index)" class="ml-2 text-red-400 hover:text-red-300">{{ $t("editTest.delete") }}</button>
                </div>
              </div>

              <div v-if="editableQuestionIndex === index">
                <!-- Editable Question Form -->
                <input v-model="editQuestionData.text" type="text" class="w-full px-3 py-2 mb-2 bg-gray-700 rounded-md" :placeholder="$t('editTest.editQuestionText')" />
                ...
                <textarea v-model="editQuestionData.explanation" class="w-full px-3 py-2 bg-gray-700 rounded-md mt-2" :placeholder="$t('editTest.explanation')"></textarea>
              </div>
              ...
              <div v-else>
                ...
                >
                  {{ option }} <span v-if="optIndex === question.correctAnswer" class="ml-2">{{ $t("editTest.correct") }}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="border-t border-gray-700 pt-6">
            <h3 class="text-lg font-semibold mb-4">{{ $t("editTest.addNewQuestion") }}</h3>
            <textarea
                v-model="newQuestion.text"
                rows="3"
                :placeholder="$t('editTest.questionText')"
                class="w-full px-3 py-2 bg-gray-700 rounded-md focus:ring-primary-500 focus:border-primary-500"
            ></textarea>
            ...
            <textarea v-model="newQuestion.explanation" class="w-full px-3 py-2 bg-gray-700 rounded-md mt-2" :placeholder="$t('editTest.explanation')"></textarea>

            <button @click="addNewQuestion" type="button" class="w-full px-4 py-2 mt-4 bg-primary-600 text-white rounded hover:bg-primary-700">{{ $t('editTest.addQuestion') }}</button>
          </div>
        </div>
        ...
        <button type="submit" :disabled="loading || formData.questions.length === 0" class="w-full bg-primary-600 text-white px-4 py-2 rounded hover:bg-primary-700 disabled:opacity-50">
          {{ loading ? $t('editTest.updatingTest') : $t('editTest.updateTest') }}
        </button>
      </form>
    </div>
  </div>
