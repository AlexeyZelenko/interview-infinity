...
const addTest = async (testId: string) => {
  if (!authStore.user) {
    Swal.fire({
      icon: 'error',
      title: 'Oops...',
      text: 'You need to be logged in to add a test!',
    });
    return;
  }

  console.log('Adding test:', testId);
  // testStore.addTest(testId);
  // await router.push(`edit-selected-test/${testId}`);
  await router.push(`test-selected/${testId}/edit`);
};
...
onMounted(async () => {
  try {
    ...
    console.log('companyJobs:', companyJobs.value);
  } catch (err: any) {
